...
check "Tela-Dark icons present"         "[[ -d /usr/share/icons/Tela-dark ]]"
...
if UFW_STATUS=$(sudo ufw status 2>/dev/null); then
    ...
    check "deny incoming"       "sudo ufw status verbose 2>/dev/null | grep -q 'Default: deny (incoming)'"
    check "allow outgoing"      "sudo ufw status verbose 2>/dev/null | grep -q 'allow (outgoing)'"
else
...
check "NetworkManager hardened" "[[ -f /etc/systemd/system/NetworkManager.service.d/hardened.conf ]]"
...
THEME_SLUG=$(cat "$HOME/.local/state/omarchy/current/theme.name" 2>/dev/null | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
[[ -z "$THEME_SLUG" ]] && THEME_SLUG="tokyo-night"
check "icons.theme overlay"      "[[ -f $HOME/.config/omarchy/themes/$THEME_SLUG/icons.theme ]]"
check "icons.theme -> Tela"      "grep -q 'Tela-dark' $HOME/.config/omarchy/themes/$THEME_SLUG/icons.theme 2>/dev/null"
check "folder-color hook"        "[[ -x $HOME/.config/omarchy/hooks/theme-set.d/folder-color ]]"
check "yaru removed"             "! pacman -Q yaru-icon-theme &>/dev/null"
